...
namespace LGTuner.hirnugeos
{
    public class Hirnu_hub : MonoBehaviour
    {
        ...
        {
            if (EntryPoint.hirnuhubgo != null) EntryPoint.hirnuhubgo.SetActive(false);
        }
        public static void OnFactoryBuildStart(SubComplex subcomplex, LG_Zone zone)
        {
            EntryPoint.hirnuhubgo = null;
            EntryPoint.hirnuhubgo = new();
            EntryPoint.hirnuhubgo.SetActive(true);
            Debug.Log("LGTuner - hirnugeos - building hub-tile");
            EntryPoint.hirnuhubgo.SetActive(true);
            EntryPoint.hirnuhubgo.hideFlags = HideFlags.HideAndDontSave;
            EntryPoint.hirnuhubgo.layer = LayerManager.LAYER_DEFAULT;
            EntryPoint.hirnuhubgo.name = "hirnu_tile_hub";
            var geo = EntryPoint.hirnuhubgo.AddComponent<LG_Geomorph>();
            var vnv = EntryPoint.hirnuhubgo.AddComponent<AIG_VoxelNodeVolume>();

            GameObject area = new("Area A");
            area.transform.parent = EntryPoint.hirnuhubgo.transform;
            ...
            for (int i = -2; i < 3; i++)
            {
                ...
                GameObject tmpmark4 = new($"Marker {i} south");
                tmpmark4.transform.parent = markers.transform;
                tmpmark4.transform.localPosition = new(i, 0, -2.5f);
                tmpmark4.transform.localEulerAngles = new(0, 180, 0);
                var tmplock4 = tmpmark4.AddComponent<TechDataCenterMarkerProducer>();
                tmplock4.m_groupData = mgroup;
                tmplock4.m_allowFunction = true;
                tmplock4.m_markerDataBlockID = 77;
            }
            ...
            foreach (var blurgh in EntryPoint.hirnuhubgo.GetComponentsInChildren<Transform>()) blurgh.gameObject.hideFlags = HideFlags.HideAndDontSave;
        }
    }
}